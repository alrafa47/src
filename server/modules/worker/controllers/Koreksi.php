...
class Koreksi extends BaseWorker
{
    function __construct()
    {
        ...
        $this->setup(
            ...
            Config()->item('queueServer_typeKoreksi'),
            Config()->item('queueServer_formatTubeKoreksi')
        );
    }
    ...
    protected function handleJob($job)
    {
        ...
        $model = $this->model('sipas/koreksi_masuk_view', true);
        $data = $job->getBody(); // body is always array
        
        $operation = $model->create_koreksi($data);
