...
class Koreksi_masuk extends Base_Controller
{
    ...
    function update($usePayload = true)
    {
        ...
        // *pengaturan distribusi otomatis SKE diaktifkan
        $distribusi_otomatis_SKE = (bool)$pengaturan->getSettingByCode('use_auto_distribusi_keluar_eksternal');

        // // *pengaturan distribusi otomatis SME diaktifkan
        // $distribusi_otomatis_SME = (bool)$pengaturan->getSettingByCode('use_auto_distribusi_masuk_eksternal');
        ...
        // $penerima_records = $surat_stack_view->find(array(
        ...
        // // print_r($data);
        // $datasuratk = $surat->read($data['surat_id']);
        // unset($datasuratk['surat_id']);
        // foreach ($penerima_records as $penerima) {
        //     $datasuratk['surat_registrasi'] = $surat_view->generate_code();
        //     $datasuratk['surat_model'] = 1;
        //     $datasuratk['surat_unit'] = $penerima['unit_id'];
        //     $operation = $surat->insert($datasuratk, null);
        //     // $this->db->insert('surat', $datasuratk);
        //     echo $data['surat_unit'];
        //     print_r($datasuratk);
        //     echo '<br>';
        //     echo $this->no;
        //     echo '<br>';
        ...
        $operation = $koreksi_masuk->update(
            $id,
            $data,
            function ($response) use (
                ...
                $properti,
                $auto_nomor_setting,
                $auto_nomor_keluar_setting,
                $auto_distribusi_setting,
                $koreksi_masuk_log,
                $mergeData,
                $queueTube,
                $queueTubeRedis,
                $disposisi_masuk_lite_view,
                $worker_mode,
                $queuetubeDisposisi,
                ...
                if ($data['disposisi_model_sub'] == 1) { /* Jika Petikan */
                    $stack_penyetuju = $surat_stack->read(array(
                        'surat_stack_surat'  => $data['surat_id'],
                ...
                if (isset($data['baca']) and $data['baca'] === 1 and $data['disposisi_masuk_isbaca'] !== 1) {
                    if ($stack_penyetuju) {
                        /*updating stack baca*/
                        $surat_stack->update($stack_penyetuju['surat_stack_id'], array(
                ...
                if ($data['disposisi_model_sub'] == 1) { /* jika petikan */
                        ...
                        if ($data['disposisi_masuk_status'] == $surat::SETUJU_TOLAK) {
                            ...
                            if (!$next_record) {
                                ...
                                if ($data['surat_model'] === $surat::MODEL_KEPUTUSAN) {
                                    ...
                                    $surat->update(array('surat_id' => $surat_id), $updated_record, function ($response) use (
                                        $surat,
                                        $surat_log,
                                        $account,
                                        $account_id,
                    ...
                    /*get records for all penyetuju ordered by level*/
                    $penyetuju_records = $surat_stack->find(array(
                        'surat_stack_surat' => $data['surat_id'],
                        'surat_stack_model' => $surat_stack::MODEL_PENYETUJU
                    ), false, false, true, array('surat_stack_level' => 'asc'));

                    /*get level penyetuju by index of $penyetuju_records*/
                    $stack_penyetuju = $surat_stack->read(array(
                        'surat_stack_surat'  => $data['surat_id'],
                        'surat_stack_staf'   => $data['disposisi_masuk_staf'],
                        'surat_stack_model'  => $surat_stack::MODEL_PENYETUJU
                    ...
                    $key = $this->getIndex($penyetuju_records, $data);
                    $current_record = $penyetuju_records[$key];
                    $penerima_lvl = $key;
                    ...
                    if ($urut) {
                        ...
                        if ($data['disposisi_masuk_status'] == $surat::SETUJU_TOLAK) {
                            $prev_record = (array_key_exists($key - 1, $penyetuju_records)) ? $penyetuju_records[$key - 1] : null;
                            ...
                            $next_record = (array_key_exists($key + 1, $penyetuju_records)) ? $penyetuju_records[$key + 1] : null;

                            if (!$next_record) {
                                // jika penyetuju adalah penyetuju terakhir
                                $surat_id = $data['surat_id'];

                                if ($data['surat_model'] === $surat::MODEL_KELUAR) {

                                    // *jika distribusi otomatis SKE diaktifkan 
                                    if ($distribusi_otomatis_SKE) {
                                        ...
                                        if ($data['surat_nomor']) {
                                            ...
                                            $gen_nomor = $surat_view->generate_nomor($surat_id, 'keluar', $penyetuju_records[$penerima_lvl]['surat_stack_staf'], true, $tgl);
                                            if ($auto_nomor_keluar_setting) {
                                                ...
                                                $surat_nomor_urut = $gen_nomor['digit'];
                                                $surat_nomor_tgl = $now;
                                                $surat_nomor_staf = $penyetuju_records[$penerima_lvl]['surat_stack_staf'];
                                                $surat_nomor_profil = $penyetuju_records[$penerima_lvl]['surat_stack_profil'];
                                            } else {
                                                $surat_nomor_tgl = NULL;
                                                $surat_nomor_format = NULL;
                                                $surat_nomor_otomatis = NULL;
                                        ...
                                        $surat->update(array(
                                            ...
                                            'surat_setuju_staf'     => $penyetuju_records[$penerima_lvl]['surat_stack_staf'],
                                            'surat_setuju_profil'   => $penyetuju_records[$penerima_lvl]['surat_stack_profil'],
                                            'surat_distribusi_tgl'  => $now,
                                            'surat_distribusi_staf' => $penyetuju_records[$penerima_lvl]['surat_stack_staf'],
                                            'surat_distribusi_profil' => $penyetuju_records[$penerima_lvl]['surat_stack_profil'],
                                            'surat_nomor'           => $surat_nomor,
                                            'surat_nomor_asli'      => $surat_nomor_asli,
                                            'surat_nomor_urut'      => $surat_nomor_urut,
                                            'surat_nomor_backdate'  => $surat_nomor_backdate,
                                        ...
                                        //* jika jenis pengiriman ke Dalam Perusahaan
                                        if ($data['surat_keluar_type'] == 1) {
                                        ...
                                        // *jika distribusi otomatis SKE nonaktif 
                                        ...
                                        $surat->update(array(
                                            ...
                                            'surat_setuju_staf'     => $penyetuju_records[$penerima_lvl]['surat_stack_staf'],
                                            'surat_setuju_profil'   => $penyetuju_records[$penerima_lvl]['surat_stack_profil'],
                                            'surat_distribusi_tgl'  => null,
                                            'surat_distribusi_staf' => null,
                                            'surat_distribusi_profil' => null,
                                            'surat_nomor'           => $surat_nomor,
                                            'surat_nomor_asli'      => $surat_nomor_asli,
                                            'surat_nomor_urut'      => $surat_nomor_urut,
                                            'surat_nomor_backdate'  => $surat_nomor_backdate,
                                    ...
                                    if ($data['surat_nomor']) {
                                        ...
                                        $gen_nomor = $surat_view->generate_nomor($surat_id, 'ikeluar', $penyetuju_records[$penerima_lvl]['surat_stack_staf']);
                                        if ($auto_nomor_setting) {
                                            $surat_nomor = $gen_nomor['nomor'];
                                            $surat_nomor_asli = $gen_nomor['nomor'];
                                            $surat_nomor_tgl = $now;
                                    ...
                                    $updated_record = array(
                                        ...
                                        'surat_selesai_staf'     => $penyetuju_records[$penerima_lvl]['surat_stack_staf'],
                                        'surat_selesai_profil'   => $penyetuju_records[$penerima_lvl]['surat_stack_profil'],
                                        'surat_nomor'            => $surat_nomor,
                                        'surat_nomor_asli'       => $surat_nomor_asli,
                                        'surat_nomor_tgl'        => $surat_nomor_tgl,
                                        'surat_nomor_staf'       => $surat_nomor_staf,
                                    ...
                                    $surat->update(array(
                                        'surat_id' => $surat_id
                                    ), $updated_record, function ($response) use (
                                        $surat,
                                        $surat_log,
                                        $account_id,
                                        $surat_nomor,
                                    ...
                                    if ($auto_nomor_keluar_setting || $data['surat_nomor']) {
                                        ...
                                        if (!empty($stack)) {
                                            $koreksi_operation = $koreksi->insert(
                                                array(
                                                    'disposisi_tgl'      => $now,
                                                    'disposisi_pelaku'   => $penyetuju_records[$penerima_lvl]['surat_stack_staf'],
                                                    'disposisi_pelaku_profil' => $penyetuju_records[$penerima_lvl]['surat_stack_profil'],
                                                    'disposisi_staf'     => $penyetuju_records[$penerima_lvl]['surat_stack_staf'],
                                                    'disposisi_profil'   => $penyetuju_records[$penerima_lvl]['surat_stack_profil'],
                                ...
                                // jika penyetuju bukan penyetuju terakhir
                                $staf_next = $next_record['surat_stack_staf'];
                                $staf_next_profil = $next_record['surat_stack_profil'];
                            }
                        }
                        ...
                        /*create koreksi, send to next penyetuju*/ else if ($staf_next) {
                            $update_stack = $surat_stack->update(array(
                                'surat_stack_surat' => $data['surat_id'],
                                'surat_stack_staf' => $staf_next,
                                'surat_stack_model' => $surat_stack::MODEL_PENYETUJU
                        ...
                        $max_penerima = count($penyetuju_records);
                        $cek_all = array();
                        $cek_x = array();
                        ...
                        if (count($cek_all) == $max_penerima) {
                            ...
                            if ($data['surat_model'] === $surat::MODEL_KELUAR) {
                                if ($data['surat_nomor']) {
                                    ...
                                    $gen_nomor = $surat_view->generate_nomor($surat_id, 'keluar', $penyetuju_records[$penerima_lvl]['surat_stack_staf']);
                                            ...
                                            function ($response) use (
                                                ...
                                                foreach ($stack as $index => $p) {
                                                    ...
                                                    $data_diposisi_masuk = array(
                                                        ...
                                                        'penerima_id' => $penerima_id,
                                                        'penerima_jabatan' => null,
                                                        'pengirim_id' => $penyetuju_records[$penerima_lvl]['surat_stack_staf'],
                                                        'berkas' => $data['surat_useberkas'],
                                                        'tembusan' => 1,
                                                        'key_redis' => Config()->item('redisPrefix') . 'disposisi_sama:' . $inserted_data['disposisi_surat'] . '-' . $penerima_id
                                                    );
    ...
    function report()
    {
        ...
        $records = $this->m_koreksi_masuk_view->find(
            array(
    ...
    function penerima()
    {
        ...
        $account        = $me->m_account;
        $penerima_view  = $me->m_surat_stack_koreksi_view;
