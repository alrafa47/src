...
class Sla_kuis extends Base_Controller {

    protected $message = array();

	public function __construct(){
        parent::__construct();
        $this->load->model(array('sipas/sla_kuis'));
        $this->model = $this->model('sipas/sla_kuis', true);
    }
    ...
    public function read(){
        ...
        if(varGetHas('id') || varGetHas('sla_kuis_id')){
            $id = varGet('id', varGet('sla_kuis_id'));
            ...
            if(!empty($query)){
                array_unshift($filter, (object)array(
                    'type'      =>'custom',
                    'value'     => 'sla_kuis_kode LIKE "%'.$query.'%" OR sla_kuis_nama LIKE "%'.$query.'%"'
                ));
            }
