...
class Surat_keputusan extends Base_Controller {
    ...
    public function update($usePayload = true) {
        ...
        if($template){
            ...
            $operation = $surat->update($id, $data, 
                function ($response) use ($me, $nomor, $data, $id, $now, $surat, $disposisi, $disposisi_masuk, $account, $account_id, $account_record, $disposisi_masuk_view, $properti, 
                    $log, $penerima, $penyetuju, $upenyetuju, $upenerima, $surat_stack, $pengaturan, $surat_view, $temporary, $booking, $disposisi_view, $surat_log, $disposisi_masuk_log, 
                    $korespondensi, $notifikasi, $internal, $bookingNomor, $auto_nomor_setting, $auto_distribusi_setting, $tembusan, $mergeData, $queueTube, $queueTubeRedis, 
                    $koreksi_masuk_view, $stafProfil, $penyetuju_profil, $penerima_profil, $upenyetuju_profil, $upenerima_profil, $queuetubeKoreksi, $worker_mode, $petikan, $upetikan, 
                    $petikan_profil, $upetikan_profil, $utembusan, $distribute, $pegawai, $dokumen_view, $queuetubeKeputusan, $tembusan_profil, $utembusan_profil) {
                ...
                if(!empty($penyetuju)){
                    if($temporary == 1 || $distribute){
                        ...
                        $disposisi_operation = $disposisi->insert(
                            ...
                            ),null, function($response) use ($me, $disposisi, $temporary, $disposisi_masuk, $log, $account_record,
                                $disposisi_masuk_view, $surat_view, $disposisi_view, $penyetuju, $surat_stack, $data, 
                                $properti, $account, $account_id, $now, $queueTube, $queueTubeRedis, $koreksi_masuk_view, 
        ...
        foreach ($unit_recs as $d_i => $v) {
            ...
            foreach ($records as $i => &$r) {        
                ...
                $stack_penerimask = $spenerimask_view->find(
                    ...
                    null,null,null, array('surat_penerimask_level'=>'asc')
                );
    ...
    function internal_report_semua(){
        ...
        if(! $unit_recs){
            ...
            $unit_nama = ($param_unit) ? $unit_model->read($param_unit)['unit_nama'] : $this::$default_value['title'];
            $unit  = array('unit_nama'=> $unit_nama, 'records'=>array());
            $surat = array_fill_keys(array('surat_jenis', 'surat_tanggal', 'surat_nomor', 'surat_perihal', 'surat_agenda_converted', 'tahun', 'surat_kelas_kode', 'surat_kelas_nama', 'surat_unit'),
