...
class Surat_ikeluar extends Base_Controller {
    ...
    public function create($usePayload = true) {
        ...
        $operation = $surat->insert($data, null, function ($response) use (
            $me, $data, $account_id, $now, $surat, $properti, $surat_log, $stafProfil) {
    ...
    public function update($usePayload = true) {
        ...
        if($template){
            ...
            if($setuju){
                ...
                $operation = $surat->update($id, $data, function ($response)
                use ($me, $data, $account, $account_id, $account_record, $surat_log, $properti, $auto_distribusi_setting, $surat_view, $surat, $penerima, $surat_stack, 
                    $istembusan, /*$isberkas,*/ $auto_nomor_setting, $nomor, $mergeData, $id, $disposisi, $log, $disposisi_masuk, $pengaturan,
                    $disposisi_masuk_view, $now, $disposisi_view, $korespondensi, $queueTubeRedis, $penyetuju_profil, $penerima_profil, $notifikasi,
                    $upenyetuju_profil, $upenerima_profil, $stafProfil, $queuetubeKoreksi, $worker_mode) {
                    ...
                    $disposisi_operation = $disposisi->insert(
                        ...
                        function($response) use($disposisi, $disposisi_masuk, $log,
                            $disposisi_masuk_view, $surat_view, $surat_stack, $data, $surat_log, 
                ...
                $operation = $surat->update($id, $data, 
                    function ($response) use ($me, $nomor, $data, $id, $now, $surat, $disposisi, $disposisi_masuk, $account, $account_id, $account_record,
                        $disposisi_masuk_view, $properti, $log, $penerima, $penyetuju, $upenyetuju, $upenerima, $surat_stack, $pengaturan, 
                        $surat_view, $temporary, $booking, $disposisi_view, $surat_log, $disposisi_masuk_log, $korespondensi, $notifikasi,
                        $internal, $bookingNomor, $auto_nomor_setting, $auto_distribusi_setting, $istembusan, /*$isberkas,*/ 
                        $mergeData, $queueTube, $queueTubeRedis, $koreksi_masuk_view, $stafProfil, $penyetuju_profil, 
                        $penerima_profil, $upenyetuju_profil, $upenerima_profil, $queuetubeKoreksi, $worker_mode) {
                        ...
                        if($temporary == 1){
                            ...
                            $disposisi_operation = $disposisi->insert(
                                array(
                                    ...
                                    'disposisi_surat'       => $data['surat_id'],
                                    'disposisi_baca_tgl'    => $now
                                ),null, function($response) use ($me, $disposisi, $temporary, $disposisi_masuk, $log, 
                                    $disposisi_masuk_view, $surat_view, $disposisi_view, $penyetuju, $surat_stack, $data, 
                                    $properti, $account, $account_id, $account_record, $now, $queueTube, $queueTubeRedis, $koreksi_masuk_view, 
                                    $penyetuju_profil, $queuetubeKoreksi, $worker_mode, $pengaturan, $notifikasi){
    ...
    function internal_report_semua(){
        ...
        if(! $unit_recs){
            ...
            $unit_nama = ($param_unit) ? $unit_model->read($param_unit)['unit_nama'] : $this::$default_value['title'];
            $unit  = array('unit_nama'=> $unit_nama, 'records'=>array());
            $surat = array_fill_keys(array('surat_jenis', 'surat_tanggal', 'surat_nomor', 'surat_perihal', 'surat_agenda_converted', 'tahun', 'surat_kelas_kode', 'surat_kelas_nama', 'surat_unit', 'surat_buat_staf'),
