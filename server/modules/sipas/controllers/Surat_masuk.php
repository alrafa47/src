...
class Surat_masuk extends Base_Controller 
{
    ...
    public function create($usePayload = true){
        ...
        $operation = $surat->insert($data, null, function($response) 
            use($data, $surat, $properti, $account, $korespondensi, $now, 
                $surat_log, $akun, $arsip, $stafProfil){
    ...
    public function update($usePayload = true){
        ...
        $operation = $surat->update($id, $data, function($response) 
            use($data, $surat, $surat_view, $properti, $account, $akun_id, $akun, $korespondensi, $surat_stack, $penerima, $surat_log, 
                $temporary, $log, $disposisi_masuk_view, $disposisi_masuk, $model, $now, $disposisi_view, $disposisi, $setting,
                $notifikasi, $staf, $istembusan, /*$isberkas,*/ $mergeData, $id, $queueTube, $queueTubeRedis, $worker_mode, 
                $queuetubeDisposisi, $stafProfil, $penerima_profil){
            if($response[$surat->successProperty] !== true) return;
            ...
            if (empty($data['surat_korespondensi_surat'])) {
                ...
                if(empty($data['surat_korespondensi'])){
                    ...
                    null, function($r_korespondensi) use( $response, $surat, $korespondensi, $properti, $akun_id){
            ...
            if($temporary == 1){
                ...
                if(!empty($penerima)){
                    ...
                    $dpo = $this->m_disposisi->insert(
                        ...
                        function($response) use($penerima, $now, $account, $akun_id, $akun, $disposisi_masuk_view, $model, $disposisi_masuk,
                            $data, $temporary, $properti, $log, $surat_stack, $surat_view, $disposisi, $setting, $staf, $notifikasi,
                            $istembusan, /*$isberkas,*/ $queueTube, $queueTubeRedis, $worker_mode, $queuetubeDisposisi, $penerima_profil){
