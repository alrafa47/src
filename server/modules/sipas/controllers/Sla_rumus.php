...
class Sla_rumus extends Base_Controller {

    protected $message = array();

	public function __construct(){
        parent::__construct();
        $this->load->model(array('sipas/sla_rumus'));
        $this->model = $this->model('sipas/sla_rumus', true);
    }
    ...
    public function read(){
        ...
        if(varGetHas('id') || varGetHas('sla_rumus_id')){
            $id = varGet('id', varGet('sla_rumus_id'));
            ...
            if(!empty($query)){
                array_unshift($filter, (object)array(
                    'type'      =>'custom',
                    'value'     => 'sla_rumus_kode LIKE "%'.$query.'%" OR sla_rumus_nama LIKE "%'.$query.'%"'
                ));
            }
