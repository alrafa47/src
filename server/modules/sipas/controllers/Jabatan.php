...
class Jabatan extends Base_Controller
{
    ...
    public function read($section = null)
    {
        ...
        if ($section == 'tree') {
            ...
            if (!empty($id)) {
                ...
                $filter = $this->filter_unit($filter);
    ...
    public function create($usePayload = true)
    {
        ...
        $operation = $model->insert($data, null, function ($response) use ($model, $akun, $properti, $data, $path) {
    ...
    public function filter_unit($filter)
    {
        $unit = varGet('unit');
        if ($unit == 'semua') {
            return;
        ...
        array_unshift($filter, (object)array(
            'type'  => 'custom',
            //* semua termasuk unitnya sendiri akan muncul 
            'value' => "unit_id = '$unit' || unit_parent_path LIKE '%$unit%'"
            //* hanya unit bawahannya yang muncul
