...
class Sipas_model_Surat_imasuk_view extends Sipas_model_Surat_view {

    public static $use_approvalinfo_on_read = false;

    function __construct(){
        parent::__construct();
        $this->set_table_name('v_surat_imasuk');
    ...
    function generate_code($tipe = null,$scope = null,$index = false){
        ...
        $tipeInternal = $CI->model('sipas/jenis', true);
    ...
    function select($config = NULL, $fn = NULL){
        ...
        if($this::$use_approvalinfo_on_read){
            $records[$this->field_approvalinfo] = $this->approvalInfo();
    ...
    public function approvalInfo(){
        ...
        $filters = array(
            '('.$this->field_surat_retensi.' >= "'.$hariini.'" OR '.$this->field_surat_retensi.' IS NULL )' => null
        );
        
        $filters_retensi = array(
            '('.$this->field_surat_retensi.' < "'.$hariini.'" AND '.$this->field_surat_useretensi.' = 1 )' => null
        );
        
        $response[$this->field_approvalinfo_total] = $this->count_exist($filters);
        
        $response[$this->field_approvalinfo_today] = $this->count_exist(array_merge(
            $filters,
            array(
                'DATE('.$this->field_surat_tanggal.')' => date('Y-m-d')
            )
        ));
        $response[$this->field_approvalinfo_retensi] = $this->count_exist(array_merge(
            $filters_retensi
        ));
