Ext.define('SIPAS.controller.Sipas.disposisi.session.respon.Popup', {
    ...
    refs: [
        ...
        { ref: 'aksiCombo',         selector: 'sipas_disposisi_session_respon_popup > #form combobox[name=disposisi_masuk_aksi]' },
        { ref: 'aksiPesan',         selector: 'sipas_disposisi_session_respon_popup > #form #textAksi' }
    ],
                    ...
                    if(button == 'yes'){
                        ...
                        record.saveAksi({
                            ...
                            callback: function(staf, operation, success){
                                ...
                                var res = Ext.decode(operation.response.responseText, true) || {};
                                if(success){
                                    $this.aksiID = res.data.disposisi_masuk_aksi;
                                    mainview.close();
                                    $helper.showMsg({success: true, message: 'Berhasil menyimpan respon'});
                                    Ext.callback(mainview.callback, mainview, [true, staf, eOpts]);
                                } else {
                                    $helper.showMsg({success: false, message: 'Gagal menyimpan respon'});
                                }
                            }
                        });
                    }
