Ext.define("SIPAS.controller.Sipas.keluar.agenda.Prop", {
  ...
  controllerPopupPenerima: "Sipas.surat.penerima.stack.Popup",
  ...
  init: function (application) {
    this.control({
      ...
      "sipas_surat_agenda_prop #panePenerimaKeluar[propType=keluar]": {
        /*
         * setting onPenerimaKeluar_LoadAssociate on app\controller\Sipas\surat\agenda\Prop.js
         */
        loadassociate: this.onPenerimaKeluar_LoadAssociate,
      },
      ...
      "sipas_surat_agenda_prop #btnDistribusiSuratKeluar": {
        click: this.onButtonDistribusiSuratKeluar_Click,
      },
  ...
  onChange_Tgl: function (field, newValue, oldValue, eOpts) {
    ...
    if (
      !surat_nomor &&
      surat_tanggal < pembuatan_tanggal &&
      (view.mode != "view" || view.mode != "ubah" || view.mode != "lihat")
    ) {
  ...
  onButtonDistribusiSuratKeluar_Click: function (button, e, options) {
    var $this = this,
      $app = $this.getApplication(),
      $session = $app.getSession(),
  ...
  onChange_TglSession: function (field, newValue, oldValue, eOpts) {
    var $this = this,
      ...
      pembuatan_tanggal =
        new Date(record.get("surat_properti_buat_tgl")) || new Date(Date.now()),
  ...
  onButtonSave_Click: function (button, e, eOpts, record) {
    var $this = this,
      ...
      jenis_pengiriman_type = record.get("surat_keluar_type"),
      storePenyetuju = $this.getStore($this.storePenyetuju),
      storePenerima = $this.getStore($this.storePenerima),
      storePenerimaKeluar = $this.getStore("Sipas.surat.penerimakeluar.List"),
    ...
    /*
     *jika jenis pengiriman yang pilih adalah pengiriman ke dalam perusahaan
     */
    if (jenis_pengiriman_type == "1") {
      storePenerimaKeluar.each(function (r) {
        params["pnk[]"].push(r.get("staf_id"));
        // params['b[]'].push(r.get('surat_stack_isberkas'));
        if (r.get("surat_stack_profil")) {
          params["pnk_p[]"].push(r.get("surat_stack_profil"));
        } else {
          params["pnk_p[]"].push(r.get("staf_profil"));
        }
      });
    }
    ...
    */
    if (useretensi && !retensi_tgl) {
      $helper.showMsg({
        success: false,
        message: $this.getMessage("retensi_empty"),
      });
      return;
    }
  ...
  onButtonSaveEdit_Click: function (button, e, eOpts, record) {
    ...
    if (
      Ext.Array.contains(["ubah"], view.mode) &&
      params["upn[]"].length > 50
    ) {
      $helper.showMsg({
        title: "Info",
        message: $this.getMessage("receiver_limit"),
  ...
  onButtonSaveSend_Click: function (button, e, eOpts, record) {
    ...
    if (
      Ext.Array.contains(["add", "edit"], view.mode) &&
      !params["py[]"].length
    ) {
      $helper.showMsg({
        success: false,
                  ...
                  if (button == "yes") {
                    ...
                    if (!form.getForm().isValid()) {
                      ...
                      if (record.get("surat_setuju") === 4) {
                        record.set({
                          surat_setuju:
                            record.self.statusPenyetujuan()
                              .PERSETUJUAN_REVISION,
                          surat_setuju_staf: stafId,
                          surat_setuju_tgl: new Date(),
                        ...
                        record.set({
                          surat_setuju:
                            record.self.statusPenyetujuan().PERSETUJUAN_PROCESS,
                          surat_setuju_staf: stafId,
                          surat_setuju_tgl: new Date(),
                          /*agenda keluar didistribusikan ketika dikirim lewat ekspedisi
                                                    'surat_distribusi_staf' : stafId, 
                    ...
                    $helper.saveRecord({
                      ...
                      callback: function (success, records, response) {
                        ...
                        if (success) {
                          ...
                          $helper.showMsg({
                            success: false,
                            message: res.message || "Gagal Menyimpan Surat",
                          });
                        }
                      },
                    });
            ...
            if (button == "yes") {
              ...
              if (!form.getForm().isValid()) {
                ...
                if (record.get("surat_setuju") === 4) {
                  record.set({
                    surat_setuju:
                      record.self.statusPenyetujuan().PERSETUJUAN_REVISION,
                    surat_setuju_staf: stafId,
                    surat_setuju_tgl: new Date(),
                    /*agenda keluar didistribusikan ketika dikirim lewat ekspedisi
                                        'surat_distribusi_staf' : stafId, 
  ...
  onButtonSimpanSetujui_Click: function (button, e, eOpts, record) {
    ...
    if (Ext.Array.contains(["add", "edit"], view.mode)) {
      ...
      if (penyetujuTerakhir == null) {
        $helper.showMsg({
          success: false,
          message: $this.getMessage("invalid_eselon"),
        });
        return;
      }
    }
                ...
                callback: function (button) {
                  if (button == "yes") {
                    ...
                    if (!form.getForm().isValid()) {
                      ...
                      record.set({
                        surat_setuju:
                          record.self.statusPenyetujuan().PERSETUJUAN_APPROVE,
                        surat_setuju_staf: stafId,
                    ...
                    $helper.saveRecord({
                      ...
                      callback: function (success, record, eOpts, response) {
                        ...
                        if (success) {
                          ...
                          $helper.showMsg({
                            success: true,
                            message: "Berhasil Menyetujui Surat",
                          });
                          // Ext.callback(view.callback, view, [success, record, eOpts]);
                          view.close();
                        } else {
                          ...
                          $helper.showMsg({
                            success: false,
                            message:
                              "Anda tidak bisa backdate pada tanggal ini",
                          });
  ...
  onPilihKorespondensi_Click: function (button, e, eOpts) {
    ...
    controllerLookup.launch({
      ...
      callback: function (selection) {
        $this.setKorespondensi(
          selection[0],
          $this.getMainview({ from: button })
        );
      },
    });
  },
  ...
  onButtonEkspedisiKeluar_Click: function (button, e, eOpts) {
    var $this = this,
      view = $this.getMainview({ from: button }),
      form = $this.getForm({ root: view }),
      controllerPopup = $this.getController(
        $this.controllerPopupEkspedisiKeluar
      ),
      record = form && form.updateRecord().getRecord();
  ...
  onButtonResendTembusan_Click: function (button, e, eOpts) {
    var $this = this,
      controllerResendTembusan = $this.getController(
        $this.controllerResendTembusan
      ),
      view = $this.getMainview({ from: button }),
      form = $this.getForm({ root: view }),
      record = form && form.updateRecord().getRecord();
                          ...
                          Ext.Ajax.request({
                            ...
                            success: function (response, options) {
                              ...
                              record.set({
                                ...
                                surat_nomor_tgl: records.get("surat_nomor_tgl"),
                                surat_nomor_staf:
                                  records.get("surat_nomor_staf"),
                                surat_nomor_otomatis: records.get(
                                  "surat_nomor_otomatis"
                                ),
                                surat_nomor_booking: records.get(
                                  "surat_nomor_booking"
                                ),
                                surat_nomor_format:
                                  records.get("surat_nomor_format"),
                                surat_nomor_urut:
                                  records.get("surat_nomor_urut"),
                                surat_nomor_backdate: records.get(
                                  "surat_nomor_backdate"
                                ),
                                surat_isbackdate:
                                  records.get("surat_isbackdate"),
                              });
                              cmpNomor.setValue(objres.next);
                              tanggal.setValue(records.get("surat_tanggal"));
                              btnNomor.hide();
          ...
          Ext.Ajax.request({
            ...
            success: function (response, eOpts) {
              ...
              if (!success) {
                $helper.showMsg({
                  success: false,
                  message: "Gagal mengarsipkan surat",
                });
                return;
              }
              if (success) {
                $helper.showMsg({
                  success: true,
                  message: "Berhasil mengarsipkan surat",
                });
                view.close();
                Ext.callback(view.callback, view, [success, record, eOpts]);
              }
