Ext.define("SIPAS.controller.Sipas.Home", {
  ...
  views: ["Sipas.Home", "Sipas.Viewport"],
  ...
  launch: function (config, scope) {
    if (Ext.isFunction(config))
      config = { callback: config, scope: scope || this };
  ...
  onHome_AfterRender: function (component, eOpts) {
    ...
    var sidebar = this.getCmpSidebar({ mainview: component });
    sidebar.hide();
    var header = this.getCmpMenubar({ mainview: component });
    header.hide();
  ...
  onCompLogo_AfterRender: function (component, eOpts) {
    var content = this.getApplication().getMetadata(
      "productLogoHome|productLogo|productIcon"
    );
