Ext.define('SIPAS.controller.Sipas.prioritas.List', {
    extend: 'SIPAS.controller.Sipas.base.Base',

    stores: [
        'Sipas.prioritas.semua.List',
        'Sipas.prioritas.aktif.List',
        'Sipas.prioritas.nonaktif.List',
    ...
    refs:[
        { ref: 'mainview', selector: 'sipas_prioritas_list' }
    ],

    defaultStore: 'Sipas.prioritas.semua.List',

    controllerProperty: 'Sipas.prioritas.Prop',

    init: function(application) {
        this.control({
            "sipas_prioritas_list sipas_com_button_add": {
                click: this.onButtonAdd_Click
            },
            "sipas_prioritas_list sipas_com_button_refresh": {
                click: this.onButtonRefresh_Click
            },
            "sipas_prioritas_list #Aktif": {
                select: this.onComboStatus_Select
            },
            "sipas_prioritas_list": {
                selectionchange: this.onGridpanel_SelectionChange
            },
            'sipas_prioritas_list[clickToView=true]': {
                itemclick: this.onMainview_ClickShow
            },
            'sipas_prioritas_list[dbclickToView=true]': {
    ...
    onComboStatus_Select: function(combo, selection, eOpts){
        ...
        switch(value){
            case 1:
                store.removeAll();
                store.getProxy().url = 'server.php/sipas/prioritas/aktif';
            break;
            case 2:
                store.removeAll();
                store.getProxy().url = 'server.php/sipas/prioritas/nonaktif';
            break;
            default:
                store.removeAll();
                store.getProxy().url = 'server.php/sipas/prioritas/read';
            break;
        }
