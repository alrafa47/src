Ext.define('SIPAS.controller.Sipas.internal.masuk.agenda.distribusi.jabatan.Prop', {
    ...
    api: {
        auth:   'server.php/sipas/disposisi_masuk/isexist_penerima/',
        jabatan:   'server.php/sipas/jabatan?id={id}'
    },

    refs: [
        { ref: 'mainview',          selector: 'sipas_internal_masuk_agenda_distribusi_jabatan_prop' },
        { ref: 'form',              selector: 'sipas_internal_masuk_agenda_distribusi_jabatan_prop > form' },
        { ref: 'compPenerima',      selector: 'sipas_internal_masuk_agenda_distribusi_jabatan_prop sipas_disposisi_forward_penerima_jabatan_list'},
        { ref: 'compCode',          selector: 'sipas_internal_masuk_agenda_distribusi_jabatan_prop #fieldCode'},
        { ref: 'compJenisSurat',    selector: 'sipas_internal_masuk_agenda_distribusi_jabatan_prop sipas_com_surat_pane #jenisSurat'},
        { ref: 'compDetailSurat',   selector: 'sipas_internal_masuk_agenda_distribusi_jabatan_prop sipas_com_surat_pane #suratDetail'},
        { ref: 'btnKirim',          selector: 'sipas_internal_masuk_agenda_distribusi_jabatan_prop sipas_com_button_disposisi'},
    ],
    ...
    init: function(application) {
        this.control({
            'sipas_internal_masuk_agenda_distribusi_jabatan_prop': {
                show: this.onMainview_Show,
                dosave: this.onMainview_DoSave
            },
            'sipas_internal_masuk_agenda_distribusi_jabatan_prop sipas_disposisi_forward_penerima_jabatan_list' : {
                loadassociate: this.onPenerima_LoadAssociate
            },
            'sipas_internal_masuk_agenda_distribusi_jabatan_prop sipas_disposisi_forward_penerima_jabatan_list sipas_com_button_plus': {
                click: this.onButtonTambahPenerima_Click
            },
            'sipas_internal_masuk_agenda_distribusi_jabatan_prop #toolbarAction button[action]': {
                click: this.onButtonAction_Click
            },
            'sipas_internal_masuk_agenda_distribusi_jabatan_prop sipas_com_surat_pane':{
...
//                         storePenerima.add(r);
//                     }else{
//                         var msg = $this.getMessage('receiver_exist',{id: r.get('jabatan_id')});
...
//                 });
                ...
                for(var i in selections){
                    ...
                    if(!find){
                        ...
                        var msg = $this.getMessage('receiver_exist',{id: selections[i].data.jabatan_nama});
                        $helper.showNotification(msg[0],msg[1]);
                    }
                }
            }
    ...
    onMainview_DoSave: function(mainview, button, e, eOpts){
        ...
        if(storePenerima.getCount() <= 0){
            ...
            storePenerima.each(function(r){
                params['user[]'].push(r.get('jabatan_id'));
                params['nama[]'].push(r.get('jabatan_nama'));
    ...
    isExist: function(jabatan_id, surat_id){
        var $this = this;
    ...
    onPenerima_LoadAssociate: function(record, form, cmp){
        var $this = this,
            ...
            list = $this.getCompPenerima({root:mainview}),
            storePenerima = list.getStore() || $this.getListPenerima().getStore(),
            // store = record.fetchPenerima(),
            surat = mainview.surat,
            store = surat.fetchStackDisposisi(); 

        storePenerima.removeAll();
        store.load(function(){
            store.each(function(record){
                record.getJabatan(function(jabatan){
                    var find = storePenerima.findRecord('jabatan_id', jabatan.get('jabatan_id'));
                    if(!find){
                        storePenerima.addSorted(jabatan);
                    }
                });
            });
        });
