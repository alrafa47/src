Ext.define('SIPAS.controller.Sipas.surat.petikan.List', {
    ...
    refs :[
        { ref: 'mainview', selector: 'sipas_surat_petikan_list' },
        { ref: 'form',     selector: 'sipas_surat_petikan_list > form' },
        { ref: 'cbUrut',   selector: 'sipas_surat_petikan_list > toolbar #petikanUrut' }
    ],

    controllerLookupPenerima        : 'Sipas.staf.penerima.Lookup',
    controllerPropRiwayat           : 'Sipas.surat.petikan.riwayat.Popup',
    ...
    init: function(application){
        this.control({
            'sipas_surat_petikan_list':{
                rundelete: this.onMainview_RunDelete,
                runmoveup: this.onMainview_RunMoveUp,
                runmovedown: this.onMainview_RunMoveDown
            },
            'sipas_surat_petikan_list sipas_com_button_add': {
                click: this.onButtonAdd_Click
            },
            'sipas_surat_petikan_list grid': {
                selectionchange: this.onGridpanel_SelectionChange
            },
            'sipas_surat_petikan_list #toolbarRiwayat #btnRiwayatpersetujuan': {
                click: this.onButtonRiwayat_Click
            },
            'sipas_surat_petikan_list toolbar #petikanUrut': {
    ...
    launch: function(config) {
        var $this =         this,
            ...
            storeStaf =     $this.getStore($this.storePetikan),
    ...
    onButtonAdd_Click: function(button, e, eOpts){
        ...
        if (nomorSurat && record.get('surat_nomor') && (record.get('surat_setuju') == 0 || record.get('surat_setuju') == 4) && record.get('jenis_nomor_awal') == 1) {
        ...
        controllerLookup.launch({
            ...
            callback:function(selections){
                for(var i in selections){
                    var find = storePetikan.findRecord('staf_id', selections[i].data.staf_id);
                    if(!find){
                        storePetikan.add(selections[i].data);
                    }else{                 
                ...
                if(model == 6) view.setTitle('<b style="color:#04408c; font-size: 13px">Petikan ('+storePetikan.data.length+')</b>');
            }
        })
    },
    ...
    onMainview_RunDelete: function(view, rowIdx, colIdx, item, e, record, row){
        ...
        if (nomorSurat && recordsurat.get('surat_nomor') && (recordsurat.get('surat_setuju') == 0 || recordsurat.get('surat_setuju') == 4) && recordsurat.get('jenis_nomor_awal') == 1) {
            nomorLama.setValue(recordsurat.get('surat_nomor'));
            nomorSurat.setValue(null);
            btnNomor.show();
            btnSalin.show();
        }
        ...
        if(model == 6) mainview.setTitle('<b style="color:#04408c; font-size: 13px">Petikan ('+store.data.length+')</b>');
    },
