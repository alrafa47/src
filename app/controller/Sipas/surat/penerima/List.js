Ext.define("SIPAS.controller.Sipas.surat.penerima.List", {
  ...
  views: ["Sipas.surat.penerima.List"],

  models: ["Sipas.Staf"],

  stores: ["Sipas.surat.penerima.List"],
  ...
  onMainview_RunDelete: function (view, rowIdx, colIdx, item, e, record, row) {
    ...
    if (
      record.get("surat_stack_status_tgl") &&
      record.get("surat_setuju") !== "4"
    ) {
      return;
    }
  ...
  onMainview_RunMoveUp: function (view, rowIdx, colIdx, item, e, record, row) {
    ...
    if (
      (prevRecord.get("surat_stack_status_tgl") ||
        record.get("surat_stack_status_tgl")) &&
      record.get("surat_setuju") !== "4"
    ) {
      return;
  ...
  onMainview_RunMoveDown: function (
    view,
    rowIdx,
    colIdx,
    item,
    ...
    if (
      (nextRecord.get("surat_stack_status_tgl") ||
        record.get("surat_stack_status_tgl")) &&
      record.get("surat_setuju") !== "4"
    ) {
      return;
