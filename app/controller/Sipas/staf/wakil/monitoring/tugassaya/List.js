Ext.define('SIPAS.controller.Sipas.staf.wakil.monitoring.tugassaya.List', {
    ...
    refs: [
        { ref: 'mainview',          selector: 'sipas_staf_wakil_monitoring_tugassaya_list' },
        { ref: 'compApprovalInfo',  selector: 'sipas_staf_wakil_monitoring_tugassaya_list sipas_disposisi_session_read_info_pane' },
        { ref: 'compApprovalDetail',selector: 'sipas_staf_wakil_monitoring_tugassaya_list sipas_surat_penyetujuan_detail_pane' },
        { ref: 'compInfo',          selector: 'sipas_staf_wakil_monitoring_tugassaya_list #groupInfo' },
        { ref: 'cmbStatus',         selector: 'sipas_staf_wakil_monitoring_tugassaya_list #status' }
    ],
    ...
    defaultStore: 'Sipas.staf.wakil.monitoring.tugassaya.List',
    ...
    init: function(application) {
        this.control({
            "sipas_staf_wakil_monitoring_tugassaya_list sipas_com_button_refresh": {
                click: this.onButtonRefresh_Click
            },
            "sipas_staf_wakil_monitoring_tugassaya_list": {
                afterrender: this.onMainview_AfterRender
            },
            "sipas_staf_wakil_monitoring_tugassaya_list[clickToView=true]": {
    ...
    onMainview_ClickShow: function(model, selected, eOpts){
        ...
        if(!record.get('disposisi_masuk_iscabut')){
            if ((type == 'asisten' && !role_asistensi_lihat)  || (type == 'pgs' && !role_pgs_lihat)) {
                if (type == 'asisten' && role_asistensi_pengingat && !record.get('disposisi_masuk_ispengingat') && ((record.get('disposisi_model') == 0 && ((record.get('disposisi_masuk_istembusan') == 1 && record.get('disposisi_masuk_isbaca') !== 1) || (record.get('disposisi_masuk_istembusan') == 0 && (!record.get('disposisi_masuk_isterus') || record.get('disposisi_masuk_aksi') == null)))) || (record.get('disposisi_model') == 1 && (record.get('disposisi_masuk_status') == 0 && record.get('disposisi_masuk_status') == null)))) {
                    $helper.showConfirm({
