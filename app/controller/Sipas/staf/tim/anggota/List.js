Ext.define('SIPAS.controller.Sipas.staf.tim.anggota.List', {
    ...
    refs: [
        { ref: 'mainview', selector: 'sipas_staf_tim_anggota_list'}
    ],

    controllerStafLookup: 'Sipas.staf.Lookup',
    storetimAnggota: 'Sipas.staf.tim.anggota.List',

    init: function(application) {
        this.control({
            'sipas_staf_tim_anggota_list': {
            ...
            'sipas_staf_tim_anggota_list #fieldSearch': {
                select: this.onMainview_fieldSearch_Select
            },
            'sipas_staf_tim_anggota_list #buttonManual': {
    ...
    onMainview_AppendRecord: function(view, record){
        ...
        var $this       = this,
            store       = view.getStore(),
            $helper     = $this.getApplication().Helper(),
            exist       = store.findRecord('staf_tim_anggota_staf', record.get('staf_id')),
            timId       = view.up('sipas_staf_tim_anggota_form').down('[name=staf_tim_id]'),
        ...
        if(! exist){
            ...
            if ((count_before + 1) > 100){
                ...
                store.add({
                    'staf_tim_anggota_staf': record.get('staf_id'),
                    'staf_id': record.get('staf_id'),
                    'staf_tim_anggota_nama': record.get('staf_nama'),
                    'anggota_nama': record.get('staf_nama'),
                    'staf_tim_anggota_tim': vtimId,
                    'staf_tim_id': vtimId,
                    'anggota_kode': record.get('staf_kode'),
            ...
            callback:function(selections){
                for(var i in selections){
                    var rec = selections[i],
                        find = storeAnggota.findRecord('staf_tim_anggota_staf', rec.get('staf_id'));
                    ...
                    if(!find){
                        ...
                        if ((count_before + 1) > 100){
                            ...
                            storeAnggota.add({
                                'staf_tim_anggota_staf':    rec.get('staf_id'),
                                'staf_id':                  rec.get('staf_id'),
                                'staf_tim_anggota_nama':    rec.get('staf_nama'),
                                'anggota_nama':             rec.get('staf_nama'),
                                'staf_tim_anggota_tim':     timId.getValue(),
                                'staf_tim_id':              timId.getValue(),
                                'anggota_kode':             rec.get('staf_kode'),
