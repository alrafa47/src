Ext.define('SIPAS.controller.Sipas.session.profile.editor.wakil.atasan.List', {
    ...
    messages: {
        ...
        'not_allowed': ['Peringatan','Anda tidak bisa menambahkan diri anda sendiri sebagai pimpinan']
    },
    ...
    refs:[
        { ref: 'mainview',      selector: 'sipas_session_profile_editor_wakil_atasan_list' },
        { ref: 'fieldSearch',   selector: 'sipas_session_profile_editor_wakil_atasan_list #fieldSearch' }
    ],
    ...
    init: function(application) {
        this.control({
            'sipas_session_profile_editor_wakil_atasan_list': {
            ...
            'sipas_session_profile_editor_wakil_atasan_list #fieldSearch': {
                focus: this.onFieldSearch_Focus,
                select: this.onMainview_fieldSearch_Select
            },
            'sipas_session_profile_editor_wakil_atasan_list #buttonManual': {
    ...
    onFieldSearch_Focus: function(cmp, eOpts){
        ...
        if (batas_hirarki){
            store.getProxy().url = $this.getApi('batas_hirarki', {mode:'pimpinan', jabatan:$profile.jabatan_id});
        } else {
    ...
    onMainview_AppendRecord: function(view, record){
        ...
        var store = view.getStore(),
            ...
            // exist = store.findRecord('jabatan_wakil_jabatan', record.get('jabatan_id'));

        // if(exist){
            store.insert(0, {
                'jabatan_wakil_asisten': rec.get('jabatan_id'),
                'jabatan_wakil_jabatan': record.get('jabatan_id'),
                'jabatan_wakil_jabatan_nama': record.get('jabatan_nama'),
                'jabatan_wakil_jabatan_kode': record.get('jabatan_kode'),
                'jabatan_wakil_jabatan_isaktif': record.get('jabatan_isaktif')
            });
        // }
    },
    ...
    onMainview_buttonManual_Click: function(button, e, eOpts){
            ...
            callback:function(selections){
                for(var i in selections){
                    var rec = selections[i],
                        find = storeAnggota.findRecord('jabatan_wakil_jabatan', rec.get('jabatan_id'));
                    ...
                    if(!find){
                        ...
                        if ((count_before + 1) > 100){
                            ...
                            storeAnggota.add({
                                'jabatan_wakil_asisten': me.jabatan_id,
                                'jabatan_wakil_jabatan': rec.get('jabatan_id'),
                                'jabatan_wakil_jabatan_nama': rec.get('jabatan_nama'),
                                'jabatan_wakil_jabatan_kode': rec.get('jabatan_kode'),
                                'jabatan_wakil_jabatan_isaktif': rec.get('jabatan_isaktif')
                            });
                        }
                    }else{
        ...
        // for batas hirarki jabatan atur pimpinan & asisten
        if (batas_hirarki){
            config['url'] = $this.getApi('batas_hirarki', {mode:'pimpinan', jabatan:me.jabatan_induk});
        } else {
